...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    // 6. get the videos based on pipeline and options
    const result = await Video.aggregatePaginate( Video.aggregate( pipelne ), options );  // Video.aggregate( pipeline ) find the videos based on pipeline(query, sortBy, sortType, userId). // aggregatePaginate is used for pagination (page, limit)
...
const getVideoById = asyncHandler(async (req, res) => {
    ...
    // 1. Get the video id from the request params(frontend)
    try {
        const {videoId} = req.params 
        ...
        res.status(501)
        .json(new ApiError(501, {}, "Video found"))      
    }
})
